...
Deploying to a production environment (or any sensible environment) requires extra care. Before going through this tutorial, please make sure you have read and understood the concepts described in these pages:
...
Another Healthcare project could be composed of many processses in a single Living Application using many Custom Connectors.
...
For their first project, most of the teams develop all the artifacts in a single Studio project (previously named repository), i.e Git repository. While this is very convenient because you have everything in a single place, it also brings some inconsistencies when starting the 2nd project.
...
When do you need to create a 2nd project? Well usually it is a matter of business case. If the same team is implementing a Healthcare application and a Loan Management application, it is easy to see that those applications should be having their own Bonita project (e.g. Git repo).
...
### Recommended deployment policy depends on targeted environment
...
In a Production environment all the data are critical and cannot be altered or worse be lost. That is why one should take extra care before deploying anything new on this environment.
...
### Deployment policy recommended for production deployment
...
The business data model is neither scoped to a particular project nor to an application, furthermore it should never be updated in a way that causes a breaking change. A breaking change is a modification in the structure of the BDM that would break the execution of existing processes.
...
So kepp this organization file small and for testing purpose.

The Organization file should probably be ignored when deploying to a production environment.
...
If you have to deploy an Organization from a file to your production environment you should use the policy: IGNORE_DUPLICATES. This way the users already existing in production will not be modified (e.g. if the password has been renewed it will be kept as it).
...
But when deploying to a production environment, if you had already installed and configure the profile, it will replace the profile definition by the information in the file, hence you will lose the extra configuration you may have done (e.g. assign profile to groups of users).
As a result, users may lose their access to Living Applications until you restore the configuration.

So it is recommended not to deploy continuously the profiles definition. As part of your deployment make sure the profiles are installed the first time, then to not deploy them later on, or plan to reconfigure them.
...
We recommend to use the IGNORE_DUPLICATES policy as it only deploys a process when it does not already exist (same name and version)
...
The easiest way to configure the deployment is to have several deploy.json file, one per targeted environment. For example: deploy_test.json, deploy_initial_prod.json and deploy_update_prod.json.
